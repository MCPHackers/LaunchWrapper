...
public class EntrypointPatch extends GamePatch {
    ...
    public EntrypointPatch(LWGameProvider gameProvider) {
    ...
    public void process(FabricLauncher launcher, Function<String, ClassReader> classSource,
        ...
        //
        // -- CLIENT --
        // pre-1.6 (seems to hold to 0.0.11!): find the only non-static
        // non-java-packaged Object field
        // 1.6.1+: [client].start() [INVOKEVIRTUAL]
        // 19w04a: [client].<init> [INVOKESPECIAL] -> Thread.start()
        // -- SERVER --
        // (1.5-1.7?)-: Just find it instantiating itself.
        // (1.6-1.8?)+: an <init> starting with java.io.File can be assumed to be
        // definite
        // (20w20b-20w21a): Now has its own main class, that constructs the server
        // class. Find a specific regex string in the class.
        // (20w22a)+: Datapacks are now reloaded in main. To ensure that mods load
        // correctly, inject into Main after --safeMode check.
        ...
        if (gameEntrypoint == null) {
            ...
            MethodNode mainMethod = findMethod(mainClass, (method) -> method.name.equals("main")
                    && method.desc.equals("([Ljava/lang/String;)V") && isPublicStatic(method.access));
            ...
            if (type == EnvType.CLIENT && mainMethod.instructions.size() < 10) {
                // 22w24+ forwards to another method in the same class instead of processing in
                // main() directly, use that other method instead if that's the case
                ...
                for (AbstractInsnNode insn : mainMethod.instructions) {
                    ...
                    } else if (insn.getOpcode() > Opcodes.ALOAD // ignore constant and variable loads as well as NOP,
                ...
                if (invocation != null) { // simple forwarder confirmed, use its target for further processing
                    ...
                    mainMethod = findMethod(mainClass,
                            m -> m.name.equals(reqMethod.name) && m.desc.equals(reqMethod.desc));
                }
            ...
            if (gameEntrypoint == null) {
                ...
                if (newGameInsn == null && type == EnvType.SERVER) {
                    newGameInsn = (MethodInsnNode) findInsn(mainMethod,
                            insn -> (insn instanceof MethodInsnNode) && insn.getOpcode() == Opcodes.INVOKESPECIAL
                                    && hasStrInMethod(((MethodInsnNode) insn).owner, "<clinit>", "()V",
                                            "^[a-fA-F0-9]{40}$", classSource),
                            false);
                }

                // Detect 20w22a by searching for a specific log message
                if (type == EnvType.SERVER && hasStrInMethod(mainClass.name, mainMethod.name, mainMethod.desc,
                        "Safe mode active, only vanilla datapack will be loaded", classSource)) {
        ...
        if (!is20w22aServerOrHigher) {
            for (MethodNode gmCandidate : gameClass.methods) {
                ...
                if (type == EnvType.CLIENT && !isApplet && gameMethodQuality < 2) {
                    ...
                    // This is the "init()" method, or as of 19w38a is the constructor, or called
                    // somewhere in that vicinity,
                    // and is by far superior in hooking into for a well-off mod start.
                    // Also try and find a Thread.currentThread() call before the LWJGL version
                    // print.
                    ...
                    for (AbstractInsnNode insn : gmCandidate.instructions) {
                        if (insn.getOpcode() == Opcodes.INVOKESTATIC && insn instanceof MethodInsnNode) {
                            ...
                            if ("currentThread".equals(methodInsn.name) && "java/lang/Thread".equals(methodInsn.owner)
        ...
        if (type == EnvType.SERVER) {
            ...
            if (!is20w22aServerOrHigher) {
                ...
                // Server-side: Run before `server.properties` is loaded so early logic like
                // world generation is not broken due to being loaded by server properties
                // before mods are initialized.
                // ----------------
                // ldc "server.properties"
                // iconst_0
                // anewarray java/lang/String
                // invokestatic java/nio/file/Paths.get
                // (Ljava/lang/String;[Ljava/lang/String;)Ljava/nio/file/Path;
                // ----------------
                Log.debug(LogCategory.GAME_PATCH, "20w22a+ detected, patching main method...");

                // Find the "server.properties".
                LdcInsnNode serverPropertiesLdc = (LdcInsnNode) findInsn(gameMethod,
                ...
                // Find the synthetic method where dedicated server instance is created so we
                // can set the game instance.
                // This cannot be the main method, must be static (all methods are static, so
                // useless to check)
                ...
                MethodNode serverStartMethod = findMethod(mainClass, method -> {
                    if ((method.access & Opcodes.ACC_SYNTHETIC) == 0 // reject non-synthetic
                            || method.name.equals("main") && method.desc.equals("([Ljava/lang/String;)V")) { // reject
                                                                                                             // main
                                                                                                             // method
                                                                                                             // (theoretically
                                                                                                             // superfluous
                    ...
                    return methodReturnType.getSort() != Type.BOOLEAN && methodReturnType.getSort() != Type.VOID
                            && methodReturnType.getSort() == Type.OBJECT;
                });

                if (serverStartMethod == null) {
                    // We are running 20w22a, this requires a separate process for capturing game
                    // instance
                    ...
                    // This means we need to position ourselves before any dynamic registries are
                    // initialized.
                    // Since it is a bit hard to figure out if we are on most 1.16-pre1+ versions.
                    // So if the version is below 1.16.2-pre2, this injection will be before the
                    // timer thread hack. This should have no adverse effects.
                ...
                // Pass null for now, we will set the game instance when the dedicated server is
                // created.
                it.add(new InsnNode(Opcodes.ACONST_NULL));
                ...
                if (serverStartMethod == null) {
                    // FIXME: For 20w22a, find the only constructor in the game method that takes a
                    // DataFixer.
                    ...
                    // 1.16-pre1+ Find the only constructor which takes a Thread as it's first
                    // parameter
                    ...
                    serverStartIt.add(new MethodInsnNode(Opcodes.INVOKESTATIC, Hooks.INTERNAL_NAME, "setGameInstance",
            ...
            if (runDirectory == null) {
                ...
                // - level.dat is always stored in CWD. We can assume CWD is set, launchers
                // generally adhere to that.
                // - options.txt in newer Classic versions is stored in user.home/.minecraft/.
                // This is not currently handled,
                // but as these versions are relatively low on options this is not a huge
                // concern.
                 ...
                 * it.add(new MethodInsnNode(Opcodes.INVOKESPECIAL, "java/io/File", "<init>",
                 * "(Ljava/lang/String;)V", false));
                 */
                it.add(new InsnNode(Opcodes.ACONST_NULL));
                it.add(new MethodInsnNode(Opcodes.INVOKESTATIC,
                        "net/fabricmc/loader/impl/game/minecraft/applet/AppletMain", "hookGameDir",
            ...
            // Client-side:
            // - if constructor, identify runDirectory field + location, run immediately
            // after
            ...
            while (consIt.hasNext()) {
                ...
                if (insn.getOpcode() == Opcodes.PUTFIELD
                    ...
                    Log.debug(LogCategory.GAME_PATCH, "Run directory field is thought to be %s/%s",
                            ((FieldInsnNode) insn).owner, ((FieldInsnNode) insn).name);
                    ...
                    it.add(new VarInsnNode(Opcodes.ALOAD, 0));
                    it.add(new FieldInsnNode(Opcodes.GETFIELD, ((FieldInsnNode) insn).owner,
                            ((FieldInsnNode) insn).name, ((FieldInsnNode) insn).desc));
                    it.add(new VarInsnNode(Opcodes.ALOAD, 0));
                    finishEntrypoint(type, it);
    ...
    private boolean hasStrInMethod(String cls, String methodName, String methodDesc, String str,
            Function<String, ClassReader> classSource) {
        if (cls.contains("$") || (!cls.startsWith("net/minecraft") && cls.contains("/"))) {
            return false;
        }
