...
public class ClassicCrashScreen extends InjectionWithContext<MinecraftGetter> {
	...
	public boolean apply(ClassNodeSource source, LaunchConfig config) {
		...
		AbstractInsnNode insn1 = getFirst(run.instructions);
							...
							if (compareInsn(insns[1], ALOAD, 0) &&
								...
								for (MethodNode m2 : scrollArea.methods) {
									for (AbstractInsnNode insn2 = getFirst(m2.instructions); insn2 != null; insn2 = nextInsn(insn2)) {
										if (compareInsn(insn2, LDC, "selectWorld.unable_to_load") || compareInsn(insn2, LDC, "Unable to load worlds")) {
	...
	public boolean patchErrorScreen(ClassNodeSource source, ClassNode errScreen, MethodNode openScreen, MethodInsnNode translate) {
		...
		for (MethodNode m : errScreen.methods) {
			...
			for (AbstractInsnNode insn = getFirst(m.instructions); insn != null; insn = nextInsn(insn)) {
				...
				if (insn.getOpcode() == INVOKESTATIC) {
					...
					if (args[1].getInternalName().equals("java/lang/String") && args[2] == Type.INT_TYPE && args[3] == Type.INT_TYPE && args[4] == Type.INT_TYPE) {
		...
		// Cancel button patch
		patchCancelButton(source, openScreen, translate);
		source.overrideClass(errScreen);
		return true;
	}

	private boolean patchCancelButton(ClassNodeSource source, MethodNode openScreen, MethodInsnNode translate) {
		...
		for (MethodNode m : screen.methods) {
			for (AbstractInsnNode insn = getFirst(m.instructions); insn != null; insn = nextInsn(insn)) {
				AbstractInsnNode[] insns = fill(insn, 4, true);
