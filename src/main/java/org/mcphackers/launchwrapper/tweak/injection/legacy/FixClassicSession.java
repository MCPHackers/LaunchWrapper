 ...
 * FIXME mobs stop targetting player after save
 */
			...
			for (MethodNode m : minecraft.methods) {
				if (m.desc.equals("(Lcom/mojang/minecraft/level/Level;)V")) {
					for (AbstractInsnNode insn = getFirst(m.instructions); insn != null; insn = nextInsn(insn)) {
						if (compareInsn(insn, PUTFIELD, "com/mojang/minecraft/level/Level") && ((FieldInsnNode)insn).name.startsWith("particleEngine")) {
