...
public class MouseFix extends InjectionWithContext<LegacyTweakContext> {
	...
	public boolean apply(ClassNodeSource source, LaunchConfig config) {
		...
		AbstractInsnNode insn1 = getFirst(init.instructions);
		while (insn1 != null) {
			...
			if (found || compareInsn(insns[0], ALOAD, thisIndex) &&
							 compareInsn(insns[1], GETFIELD, minecraft.name, canvasName, "Ljava/awt/Canvas;") &&
					...
					for (MethodNode m : node.methods) {
						...
						while (insn != null) {
							...
							if (insn.getOpcode() == INVOKESTATIC) {
								...
								if ((compareInsn(insns2[0], INVOKESTATIC, "org/lwjgl/input/Mouse", "getDX", "()I") || compareInsn(insns2[0], INVOKESTATIC, "org/lwjgl/input/Mouse", "getDY", "()I")) &&
